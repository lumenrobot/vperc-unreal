...
ANaoCharacter::ANaoCharacter() : avatar(new Avatar())
{
	...
	positionDelta = ublas::vector<double>(3);
}
...
void ANaoCharacter::BeginPlay()
{
	...
	try {
		...
		UE_LOG(LogLumen, Error, TEXT("Error: %s"), TEXT("Exception"));
	}
...
void ANaoCharacter::Tick( float DeltaTime )
{
	...
	if (channel->BasicConsumeMessage(consumer_tag, envelope, 0)) {
		...
		UE_LOG(LogLumen, Log, TEXT("NaoCharacter Avatar %s"), *FString((boost::format("Got message: id=%1% appId=%2% correlationId=%3% contentType=%4% replyTo=%5% body=%6%") %
